...
//		findByPrice.forEach((MovieEntity n)-> mapper.map(findByPrice, n));
	...
	public MovieDto updatepriceAsperCoupon(int realPrice, MovieDto movieDto) {
		String name = movieDto.getName();
		System.out.println("MovieName: "+name);
		MovieEntity findByName = movieDao.findByName(name);
		System.out.println("MovieName :::::"+findByName.getName());
		findByName.setPrice(realPrice);
		MovieDto mapMovieDto = mapper.map(movieDao.save(findByName), MovieDto.class);
		return mapMovieDto;
